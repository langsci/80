...
\ea%5
\label{ex:4:5}
\isi{State}: A single event evaluated relative to no other event \citep[56]{Pustejovsky1991}
...
Canonical \isi{State} \citep[327]{Jackendoff1996}\\
...
(Change of state\is{State!Change of}).  This is shown below in \REF{ex:4:7}:

\ea%7
\label{ex:4:7}
Event Structure of \isi{Transition}
...
Event Structure and the pure \isi{State} shown in \REF{ex:4:6} is that the
former is linked to a \isi{Process} as shown in \REF{ex:4:7}, whilst a pure
...
\ea%8
\label{ex:4:8}
\isi{Process} Event Structure \citep[56]{Pustejovsky1991}
\begin{center}\fbox{\parbox{5cm}{\centering
\begin{forest}
[P
  [e$_1$,no edge]
...
\noindent His representation is shown below in \REF{ex:4:9}:
