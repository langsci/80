...
Below in \sectref{sec:5.2}, I will apply these criteria to a classification of property items in JC\il{Jamaican Creole}.\largerpage[-3]
...
Items which display this type of behaviour are classified here as \isi{State} consistent with the Event Structure below: 

\ea%18
\label{ex:5:18}
\isi{State} Event Structure \citep[56]{Pustejovsky1991}
\begin{center}
...
In analysing the Non-stative use of such items I would like to propose that their Event Structure is one that is similar to that of a \isi{Transition}. However, as shown below the Change of state\is{State!Change of} aspect is derived through the addition of another Event Structure level:

\ea\label{ex:5:24}
 An Event Structure representing derived \isi{Transition}
\begin{center}
 \fbox{\parbox{5cm}{\centering
\begin{forest}
...
However in the case of the transitive version (\ref{ex:5:20}c), it is a \CAUSE primitive that may be said to account for the change in the syntactic structure, namely the introduction of a Cause\slash \isi{Agent} and the transitivity that contrasts with the \isi{Stative} version in (\ref{ex:5:20}a). Regarding this, consistent with the implications of the structure in \REF{ex:5:24}, \citet{Grimshaw1990} observes that a Cause argument will be associated with the ``first sub-event which is causally related to the second sub-event'' (p.~26). Thus, the introduction of the primitive \CAUSE accounts for the change in transitivity that is seen in (\ref{ex:5:20}c) as opposed to (\ref{ex:5:20}a).
...
I analyse items displaying this behaviour as inherent States derived to express a \isi{Process} Event Structure. This is based on the primacy of \isi{State} \citep{Grimshaw1990} and the fact that inherent Processes do not also express States. In these derived Processual uses, I associate such items with an Event Structure of \isi{Process} as shown below:

\ea%27
 \label{ex:5:27}
 Event Structure of \isi{Process} (\citealt[56]{Pustejovsky1991})\\ 
